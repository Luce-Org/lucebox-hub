...
static void build_tree_causal_mask(std::vector<uint16_t> & out,
                                   int kv_start,
                                   int n_tokens,
    ...
    for (int q = 0; q < n_tokens; q++) {
        ...
        for (int j = 0; j < n_tokens; j++) {
            if (visibility[(size_t)q * n_tokens + j]) {
...
static void build_swa_tree_mask(std::vector<uint16_t> & out,
                                int kv_start,
                                int n_tokens,
                                int swa_window,
                                int ring_size,
                                const int * slot_depths,
                                const uint8_t * visibility) {
    const int kv_pad = align_up(ring_size, g_kq_stride_pad);
    const int q_pad  = align_up(n_tokens, KQ_MASK_PAD);
    out.assign((size_t)kv_pad * q_pad, F16_NEG_INF);
    ...
    for (int q = 0; q < n_tokens; q++) {
        const int abs_q = kv_start + slot_depths[q];
        const int q_lo  = std::max(0, abs_q - swa_window + 1);
        for (int abs_k = q_lo; abs_k < kv_start; abs_k++) {
            out[(size_t)q * kv_pad + (abs_k % ring_size)] = F16_ZERO;
        }
        for (int j = 0; j < n_tokens; j++) {
            if (visibility[(size_t)q * n_tokens + j]) {
                out[(size_t)q * kv_pad + ((kv_start + j) % ring_size)] = F16_ZERO;
            }
        }
    }
}

static bool compact_tree_state(GemmaTargetCache      & cache,
                               int                     committed,
    ...
    if (cache.target_feat) {
        ...
        const int    feat_w     = (int)cache.target_feat->ne[0];
        ...
        for (int d = 1; d < commit_n; d++) {
            ...
            const int src_slot = (committed + src_dfs) % cap;
            const int dst_slot = (committed + d)       % cap;
            cudaError_t ce = cudaMemcpyAsync(
                (char *)cache.target_feat->data + (size_t)dst_slot * col_stride,
                (const char *)cache.target_feat->data + (size_t)src_slot * col_stride,
                (size_t)feat_w * elt,
                cudaMemcpyDeviceToDevice, stream);
            if (ce != cudaSuccess) {
                std::fprintf(stderr, "[ddtree] target_feat compact failed: %s\n",
    ...
    for (int d = 1; d < commit_n; d++) {
        ...
        for (size_t l = 0; l < cache.attn_k.size(); l++) {
            ...
            const int src_slot  = (committed + src_dfs) % ctx_slots;
            const int dst_slot  = (committed + d)       % ctx_slots;
            const size_t slot_bytes = ck->nb[1];
            const int n_kv = (int)ck->ne[2];

            for (int h = 0; h < n_kv; h++) {
                const size_t src_off = (size_t)src_slot * ck->nb[1] + (size_t)h * ck->nb[2];
                const size_t dst_off = (size_t)dst_slot * ck->nb[1] + (size_t)h * ck->nb[2];
                cudaError_t ce = cudaMemcpyAsync((char *)ck->data + dst_off,
                                                 (const char *)ck->data + src_off,
                                                 slot_bytes,
                                                 cudaMemcpyDeviceToDevice, stream);
                if (ce != cudaSuccess) {
                    std::fprintf(stderr, "[ddtree] K compact failed: %s\n",
                                 cudaGetErrorString(ce));
                    return false;
                }
                ce = cudaMemcpyAsync((char *)cv->data + dst_off,
                                     (const char *)cv->data + src_off,
                                     slot_bytes,
                                     cudaMemcpyDeviceToDevice, stream);
                if (ce != cudaSuccess) {
                    std::fprintf(stderr, "[ddtree] V compact failed: %s\n",
                                 cudaGetErrorString(ce));
                    return false;
                }
            }
...
static bool gemma4_step(StepGraph & sg,
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (std::strncmp(argv[i], "--budget=", 9) == 0) ddtree_budget = std::atoi(argv[i] + 9);
        ...
        else if (std::strcmp(argv[i], "--pflash-alpha") == 0) sparse_fa_alpha  = (float)std::atof(require_next("--pflash-alpha"));
        ...
        else if (std::strcmp(argv[i], "--ddtree")         == 0) { /* enabled by default via --budget */ }
        else if (std::strncmp(argv[i], "--ddtree-budget=", 16) == 0) ddtree_budget = std::atoi(argv[i] + 16);
        else if (std::strcmp(argv[i], "--ddtree-budget") == 0) ddtree_budget = std::atoi(require_next("--ddtree-budget"));
        else if (std::strncmp(argv[i], "--ddtree-temp=",   14) == 0) {
            ddtree_temp = (float)std::atof(argv[i] + 14);
            if (ddtree_temp <= 0.0f) ddtree_temp = 1.0f;
        }
        else if (std::strcmp(argv[i], "--ddtree-no-chain-seed") == 0) ddtree_chain_seed = false;
    ...
    std::printf("[cfg] model=%s draft=%s method=%s gpu=%d ctx=%d n_predict=%d kv_k=%s kv_v=%s "
                ...
                "draft_max=%d adaptive=%d draft_kv_cap_override=%d pflash=%d sparse_fa_alpha=%.3f\n",
                model_path.c_str(),
                ...
                (int)use_sparse_fa, sparse_fa_alpha);
    ...
    // dispatched from build_gemma4_graph (full-attention layers, use_sparse_fa=true)
        ...
        while (std::getline(std::cin, line)) {
            ...
            int last_logit_tok = -1;
            {
                ...
                for (int cs = 0; cs < n_prompt; cs += chunk_size) {
                    ...
                    if (!gemma4_step(sg, w, cache, backend,
                                           cs, chunk_n, need_mask,
                                           /*capture=*/true,
                                           use_sparse_fa, sparse_fa_alpha,
                                           fa_window,
            ...
            while (n_generated < n_gen) {
                ...
                if (!gemma4_step(sg, w, cache, backend,
                                       ...
                                       /*with_mask=*/true,
                                       /*capture=*/false,
                                       /*use_sparse_fa=*/false, sparse_fa_alpha,
                                       fa_window)) {
    ...
    for (int bench_iter = 0; bench_iter < bench_runs; bench_iter++) {
        ...
        double prefill_t1 = now_ms();
        {
            ...
            const double prefill_ms  = prefill_t1 - prefill_t0;
            {
                ...
                std::printf("[prefill] %d tokens in %.1f ms (%.1f tok/s) "
                            ...
                            use_sparse_fa ? "+pflash" : "", chunk_size, last_logit_tok);
            }
        }
        ...
        if (have_draft) {
            ...
            const int verify_cap = std::max(dw.block_size, ddtree_enabled ? ddtree_budget + 1 : 0);
            ...
            while ((int)generated.size() < n_predict) {
                ...
                if (ddtree_enabled) {
                    ...
                    if (!gemma4_step(sg, w, cache, backend,
                                           ...
                                           use_sparse_fa, sparse_fa_alpha, fa_window)) {
                        std::fprintf(stderr, "[spec] ddtree verify build failed\n");
                        return 1;
                    }

                    std::vector<int32_t> flat_tokens(N_actual, 0);
                    flat_tokens[0] = cur_tok;
                    for (int i = 0; i < tree.n_nodes; i++) {
                        flat_tokens[i + 1] = tree.token_ids[i];
                    }
                    if (!embed_tokens_batch(w, flat_tokens.data(), N_actual,
                                            sg.inp_embed, backend)) {
                        return 1;
                    }

                    std::vector<int32_t> pos(N_actual, committed);
                    std::vector<int> slot_depths(N_actual, 0);
                    for (int i = 1; i < N_actual; i++) {
                        slot_depths[i] = tree.depths[i - 1];
                        pos[i] = committed + slot_depths[i];
                    }
                    ggml_backend_tensor_set(sg.positions, pos.data(), 0,
                    ...
                    if (sg.attn_mask && sg.attn_mask->buffer) {
                        std::vector<uint16_t> mask_buf;
                        build_tree_causal_mask(mask_buf, committed, N_actual,
                                               tree.visibility.data());
                        ggml_backend_tensor_set(sg.attn_mask, mask_buf.data(), 0,
                                                sizeof(uint16_t) * mask_buf.size());
                    }
                    if (sg.swa_mask && sg.swa_mask->buffer) {
                        const SwaView swa_view = compute_swa_view(committed, N_actual,
                                                                    w.swa_window, cache.swa_ctx_alloc);
                        std::vector<uint16_t> swa_buf;
                        build_swa_tree_mask(swa_buf,
                                            /*kv_start*/ committed,
                                            /*n_tokens*/ N_actual,
                                            /*swa_window*/ w.swa_window,
                                            /*ring_size*/ swa_view.effective_win_len,
                    ...
                    const double verify_t0 = now_ms();
                    {
                        auto st = ggml_backend_graph_compute(backend, sg.gf);
                        if (st != GGML_STATUS_SUCCESS) {
                            std::fprintf(stderr, "[spec] ddtree verify compute failed: %d\n", (int)st);
                            return 1;
                        }
                    }
                    ...
                    for (int i = 0; i < commit_n; i++) {
                        ...
                        std::printf("%d ", tok);
                        std::fflush(stdout);
                        if (IS_EOS_TOK(tok, w)) { hit_eos = true; break; }
                    }
                    commit_n = emitted_n;

                    const double commit_t0 = now_ms();
                    if (!compact_tree_state(cache, committed, accepted, commit_n)) {
                        return 1;
                    }
                    {
                        DraftKVPrefillGraph cpkg;
                        if (!build_draft_kv_prefill(cpkg, dw, cache, backend, commit_n)) {
                            std::fprintf(stderr, "[spec] ddtree draft KV prefill build failed\n");
                            return 1;
                        }
                        copy_target_feat_bf16_to_f32(backend, cache.target_feat,
                            cpkg.target_feat,
                        ...
                        {
                            std::vector<int32_t> cpos(commit_n);
                            for (int i = 0; i < commit_n; i++) cpos[i] = committed + i;
                            ggml_backend_tensor_set(cpkg.positions, cpos.data(), 0,
                                                    sizeof(int32_t) * commit_n);
                        }
                        auto cst = ggml_backend_graph_compute(backend, cpkg.gf);
                        if (cst != GGML_STATUS_SUCCESS) {
                            std::fprintf(stderr, "[spec] ddtree draft KV prefill compute failed\n");
                    ...
                    const double avg_accept = (total_draft_steps > 0)
                        ? (double)total_accepted / total_draft_steps : 0.0;
                    std::printf("[step %d] accept=%d/%d tree_N=%d avg=%.1f "
                                "draft_ms=%.2f verify_ms=%.2f kv_ms=%.2f refill_ms=%.2f\n",
                                total_draft_steps, commit_n, q_len, N_actual, avg_accept,
                                draft_t1 - draft_t0, verify_t1 - verify_t0,
                                commit_t1 - commit_t0, refill_ms);
                    adaptive.observe(commit_n, q_len, total_draft_steps);
