...
static void t1_per_layer_bytes_active_qwen3() {
    ...
    assert(got == expected && "active layer bytes mismatch");
...
static void t3_slim_ee7_savings() {
    ...
    assert(savings >= min_savings && "slim mode must save >= 800 MB");
...
static void t5_env_not_set_no_skip() {
    ...
    assert(full1 == full2 && "full model size must be deterministic");
