#!/usr/bin/env bash
# Day 5: Like-vs-like A/B/C bandit vs fixed-keep validation.
# All three conditions use the SAME claude_code harness and SAME prompt file.
# Condition C uses PFLASH_SESSION_ID to trigger the session-inject proxy
# which injects extra_body.session_id into every /v1/messages request.
set -euo pipefail

WORKTREE="/home/peppi/Dev/lucebox-hub/.claude/worktrees/pflash-auto"
RESULTS_DIR="$WORKTREE/dflash/bench/results/2026-05-22_mvp_day5"
...
PROMPT_FILE="$HARNESS_DIR/prompts/decode_check.txt"
...
mkdir -p "$RESULTS_DIR"
echo "=== Day 5 A/B/C start $(date -Is) ===" | tee "$RESULTS_DIR/run.log"
...
run_condition() {
    ...
    # Pass all variables explicitly to the inner script via env; use quoted
    # heredoc delimiter so the outer shell does NOT expand any $VARS inside.
...
INNER
    ...
    # accept_rate: from server log spec-decode line  e.g. "accepted=114/432 (26.4%)"
    local ar; ar=$(grep 'spec-decode' "$slog" 2>/dev/null | \
        grep -oE '\(([0-9.]+)%\)' | tail -1 | tr -d '()%' || echo "N/A")
    [[ -z "$ar" ]] && ar="N/A"

    # drafter_fwd timing: from "[drafter] forward+score in X.XXXs" — convert to ms
...
run_condition "C_bandit"     "0.10" "claude_code_day5_s1"

echo "=== Day 5 done $(date -Is) ===" | tee -a "$RESULTS_DIR/run.log"
